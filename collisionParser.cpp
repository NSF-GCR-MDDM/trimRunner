...
//Essentially what is written to our tree
struct TrackEvent {
  float energy;
  std::vector<int16_t> ixs, iys, izs;
  std::vector<uint8_t> nVacs;
};

//Functions
std::vector<TrackEvent> ProcessThrow(CascadeData& data, float& energy, vector<float>& xs, vector<float>& ys, vector<float>& zs, vector<short>& nVacs, float startOffset_nm, float clusteringDistance_nm, float binSize_keV, int maxEntriesPerBin, float maxEnergy_keV);
...
int main(int argc, char* argv[]) {
  ...
  std::vector<TrackEvent> allEvents;
  ...
  trimTree->Branch("energy_keV", &energy, "energy_keV/F");
  ...
  while (getline(infile, line)) {
    ...
    //elif "Prime Recoil" in line, enable tracking. If "Summary" in line, disable tracking and process the event
      ...
      std::vector<TrackEvent> newEvents = ProcessThrow(data,energy,xs,ys,zs,nVacs,startOffset_nm,clusteringDistance_nm,binSize_keV,maxEntriesPerBin,maxEnergy_keV);
      //put them in our vector of all events
      allEvents.insert(allEvents.end(),make_move_iterator(newEvents.begin()),make_move_iterator(newEvents.end()));
  ...
  for (const auto& evt : allEvents) {
    ...
    for (size_t j = 0; j < evt.ixs.size(); ++j) {
      xs.push_back(evt.ixs[j] * clusteringDistance_nm);
      ys.push_back(evt.iys[j] * clusteringDistance_nm);
      zs.push_back(evt.izs[j] * clusteringDistance_nm);
      nVacs.push_back(evt.nVacs[j]);
    }
  ...
  for (size_t i = 0; i < data.primaryEnergies.size() - 1; i++) {
    ...
    if (data.primaryEnergies[i]<=maxEnergy_keV) {
      ...
      if (energyBinCounter[binIndex] <= maxEntriesPerBin) {
        ...
        //Rotate & cluster
        std::map<std::tuple<int, int, int>, uint8_t> clusters;    
        ...
        if (!clusters.empty()) {
            TrackEvent evt;
            evt.energy = data.primaryEnergies[i];
            for (const auto& [key, count] : clusters) {
                auto [ix, iy, iz] = key;
                evt.ixs.push_back(ix);
                evt.iys.push_back(iy);
                evt.izs.push_back(iz);
                evt.nVacs.push_back(count);
            }
